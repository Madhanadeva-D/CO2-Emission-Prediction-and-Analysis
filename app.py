...
.stApp {
    background: linear-gradient(135deg, #0a0f0a 0%, #0d1a0e 50%, #0a120f 100%);
    color: #e8f5e9;
}

[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #0d1f10 0%, #061008 100%);
    border-right: 1px solid #1e4d22;
}
[data-testid="stSidebar"] * { color: #a5d6a7 !important; }

[data-testid="stMetric"] {
    background: linear-gradient(135deg, #0d2610 0%, #112b14 100%);
    border: 1px solid #2e7d32;
    border-radius: 12px;
    padding: 16px 20px;
    box-shadow: 0 0 20px rgba(46,125,50,0.15);
}
[data-testid="stMetricLabel"] { color: #81c784 !important; font-size: 0.78rem; letter-spacing: 0.1em; text-transform: uppercase; }
[data-testid="stMetricValue"] { color: #a5d6a7 !important; font-family: 'DM Mono', monospace; font-size: 1.8rem; }

[data-testid="stSlider"] > div > div > div > div { background: #2e7d32 !important; }
[data-testid="stSlider"] label { color: #81c784 !important; font-size: 0.85rem; letter-spacing: 0.05em; }
...
.prediction-box .label { font-size: 0.85rem; letter-spacing: 0.18em; text-transform: uppercase; color: #a5d6a7; margin-bottom: 8px; }
.prediction-box .value { font-family: 'DM Mono', monospace; font-size: 3.2rem; font-weight: 700; color: #ffffff; line-height: 1; }
.prediction-box .unit  { font-size: 1rem; color: #81c784; margin-top: 6px; }

.badge { display: inline-block; border-radius: 30px; padding: 6px 20px; font-size: 0.82rem; font-weight: 600; letter-spacing: 0.08em; margin-top: 14px; }
.badge-low    { background: #1b5e20; color: #a5d6a7; border: 1px solid #4caf50; }
.badge-medium { background: #e65100; color: #ffcc80; border: 1px solid #ff9800; }
.badge-high   { background: #b71c1c; color: #ef9a9a; border: 1px solid #f44336; }

.section-title { font-size: 0.75rem; letter-spacing: 0.2em; text-transform: uppercase; color: #4caf50; margin-bottom: 12px; padding-bottom: 6px; border-bottom: 1px solid #1e4d22; }

.info-row { display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid #1a3a1c; font-size: 0.88rem; }
.info-row .key { color: #66bb6a; }
.info-row .val { font-family: 'DM Mono', monospace; color: #c8e6c9; }
...
st.markdown(f'<p style="color:#66bb6a;font-size:0.9rem;margin-top:-12px;margin-bottom:24px;">Gradient Boosting · R² = {r2:.4f}</p>', unsafe_allow_html=True)
...
with col_right:
    ...
    st.markdown('<span style="font-size:0.75rem;color:#4caf50;letter-spacing:0.15em;text-transform:uppercase;">Emission Gauge</span>', unsafe_allow_html=True)
    st.progress(min(gauge, 1.0))
    st.markdown(f'<div style="display:flex;justify-content:space-between;font-size:0.75rem;color:#455a64;font-family:monospace;margin-top:4px;"><span>{y_min:.0f}</span><span>{y_max:.0f} g/km</span></div>', unsafe_allow_html=True)
...
st.markdown("""
<hr style="border:1px solid #1e4d22; margin-top:20px;">

<div style="text-align:center; font-size:1rem; color:#81c784;">
    Developed by <b>Madhanadeva D</b> · 
    <a href="https://github.com/Madhanadeva-D/Madhanadeva-D" target="_blank" style="color:#4caf50; text-decoration:none;">
        GitHub
    </a>
</div>
""", unsafe_allow_html=True)
